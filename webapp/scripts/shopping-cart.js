...
function populateShoppingCartTable(cartItems) {
    console.log("populateShoppingCartTable: populating cart table");
    ...
    for (let cartItem of cartItems) {
        ...
        rowHTML += "<span class='quantity'>" + cartItem["quantity"] + "</span>";
        rowHTML += "<button class='btn-increase' data-id='" + cartItem["movie_id"] + "'>+</button>";
        rowHTML += "</td>";
        rowHTML += "<td class='total-price'>" + (cartItem["quantity"] * cartItem["movie_price"]) + "</td>";
...
$(document).on("click", ".btn-decrease", function() {
    ...
    fetchCart("?action=decrement&movie-id=" + movieId)
});

// Event listener for increasing quantity
$(document).on("click", ".btn-increase", function() {
    let movieId = $(this).data("id");;
    fetchCart("?action=increment&movie-id=" + movieId)
});
